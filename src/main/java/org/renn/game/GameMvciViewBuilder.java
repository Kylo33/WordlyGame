...
public class GameMvciViewBuilder implements Builder<Region> {
    ...
    private void showStatsStage() {
        ...
        for (int i = 0, c = freqList.size() - 1; i < c; i++) {
            data.getData().add(new XYChart.Data<>(freqList.get(i), String.valueOf(i + 1)));
        }

        BarChart<Number, String> chart = new BarChart<>(distributionOfGuessCount, numOfGuesses);
    ...
    private void addPopups(StackPane result) {
        model.getNotificationSystem().addMessageReceiver(message -> {
